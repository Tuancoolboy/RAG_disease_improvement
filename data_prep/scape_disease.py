...
def is_toc_tag(tag: Tag) -> bool:
    if tag.find_parent(id="ftwp-container-outer") is not None:
        return True
    return "ftwp-item" in (tag.get("class") or [])


def is_non_content_tag(tag: Tag) -> bool:
    if tag.find_parent("footer") is not None:
        return True
    for class_name in ("slide_doitac", "list_hethong"):
        if tag.find_parent(class_=class_name) is not None:
            return True
    return False


def is_hospital_system_tag(tag: Tag) -> bool:
    for ancestor in [tag, *tag.parents]:
        if not isinstance(ancestor, Tag):
            continue
        classes = ancestor.get("class") or []
        if "content_insert" not in classes and "alert-info" not in classes:
            continue
        ancestor_text = clean_text(ancestor.get_text(" ", strip=True)).upper()
        if "HỆ THỐNG BỆNH VIỆN ĐA KHOA TÂM ANH" in ancestor_text:
            return True
    return False


def is_reference_tag(tag: Tag) -> bool:
...
def extract_body_lines(soup: BeautifulSoup, title_tag: Tag | None) -> list[str]:
    ...
    for tag in title_tag.find_all_next(["p", "h2", "h3", "li"]):
        ...
        if is_toc_tag(tag):
            continue
        if is_hospital_system_tag(tag):
            continue
        if is_non_content_tag(tag):
            break
